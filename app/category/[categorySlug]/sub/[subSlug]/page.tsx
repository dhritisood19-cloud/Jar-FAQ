...
import { LocaleCategoryName, LocaleSubcategoryName, LocaleQuestionText } from "../../../../components/LocaleText";
      ...
      <div className="relative py-8 px-4 overflow-hidden bg-gradient-to-br from-violet-50 via-white to-blue-50">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <nav className="flex items-center gap-1 text-sm mb-5 flex-wrap text-gray-400">
            ...
            <ChevronRightIcon sx={{ fontSize: 16 }} />
            <span className="text-gray-700 font-medium"><LocaleSubcategoryName subcategoryId={subcategory.id} english={subcategory.name} /></span>
          </nav>
          <LocaleSubcategoryName subcategoryId={subcategory.id} english={subcategory.name} as="h1" className="text-xl sm:text-2xl font-bold text-gray-900 mb-2" />
        ...
        {subcategory.questions.map((q) => (
          ...
          >
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <LocaleQuestionText questionId={q.id} english={q.question} className="text-sm text-gray-700 group-hover:text-violet-600 transition-colors line-clamp-2" />
            </div>
      ...
      {category.subcategories.length > 1 && (
        <div className="max-w-3xl mx-auto px-4 pb-8">
          <div className="rounded-2xl bg-gradient-to-br from-violet-50 to-blue-50 p-5">
            <h2 className="text-sm font-bold text-gray-800 mb-3">More in <LocaleCategoryName categoryId={category.id} english={category.faqCategory} /></h2>
                ...
                .map((s) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-gray-800 group-hover:text-violet-600 transition-colors"><LocaleSubcategoryName subcategoryId={s.id} english={s.name} /></p>
